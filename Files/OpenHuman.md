...
Skills runtime uses **QuickJS** (`rquickjs`) in **`src/openhuman/skills/`** (e.g. `qjs_skill_instance.rs`, `qjs_engine.rs`), not V8/deno\_core in this repository.
...
-   **Broadcast events** (`publish_global` / `subscribe_global`) — fire-and-forget notification. One publisher, many subscribers, no return value. Use when a module needs to *announce* something happened and other modules may react independently.
...
Compiling the Rust core on Linux requires these system packages beyond the basics: `libasound2-dev libxi-dev libxtst-dev libxdo-dev libudev-dev libssl-dev clang cmake pkg-config libstdc++-14-dev`
...
OpenHuman is een desktopapplicatie die gebruikmaakt van React en Tauri v2, met een Rust-core. Het biedt een AI-gestuurde assistent voor gemeenschappen en is ontworpen om een gebruiksvriendelijke interface te bieden voor interactie met verschillende vaardigheden en functionaliteiten.

### Kernkenmerken van OpenHuman

-   **Platform & technologie**
    
    -   Desktopapplicatie voor **Windows, macOS en Linux**.
    -   Frontend gebouwd met **React** (Vite) in de `app/` workspace.
    -   **Tauri v2** als desktop host voor windowing, IPC en procesbeheer.
    -   **Rust-core** (`openhuman_core`) als sidecar-binary, aangesproken via JSON-RPC/HTTP.
    -   **QuickJS-gebaseerde skills-runtime** (via `rquickjs`) voor sandboxed skills.
-   **Doel & gebruik**
    
    -   AI-gestuurde assistent gericht op **teams en gemeenschappen**.
    -   Ondersteunt verschillende **skills** en **agent-capabilities** via een externe skills registry.
    -   Scheiding tussen **businesslogica in Rust** en **UX/flows in React/Tauri**.

### Overzicht van de documentatie

Het document biedt een uitgebreide gids voor bijdragers en ontwikkelaars. De belangrijkste onderdelen zijn:

-   **Repository-indeling**
    
    -   `app/`: React-frontend, Tauri-shell, Vitest-tests.
    -   `src/` (root): Rust-core (`openhuman_core` library + `openhuman-core` CLI/sidecar).
    -   `gitbooks/developing/`: Architectuur- en contributor-docs (frontend, Tauri-shell, testing, release, skills).
    -   Externe **skills registry**: `tinyhumansai/openhuman-skills` (niet vendored in deze repo).
-   **Runtime-scope & architectuur**
    
    -   Desktop-only Tauri-host; geen mobiele targets in `app/src-tauri`.
    -   Core-binary draait als **sidecar**; UI communiceert via HTTP JSON-RPC (`core_rpc_relay` / `core_rpc`).
    -   Duidelijke scheiding tussen **core-domeinen** (Rust) en **shell/UX** (TypeScript/React).
-   **Configuratie**
    
    -   `.env.example` (root) voor Rust-core, Tauri-shell, logging, backend-URL, proxies, storage, AI-overrides.
    -   `app/.env.example` voor `VITE_*` frontend-variabelen (core RPC URL, Sentry, skills repo, dev helpers).
    -   Gecentraliseerde frontend-config in `app/src/utils/config.ts`.
    -   Rust-config via een TOML-gebaseerde `Config`\-struct met env-overrides.
-   **Testmethoden & kwaliteit**
    
    -   **Unit tests (frontend)**: Vitest (`pnpm test`, `pnpm test:coverage`).
    -   **E2E-tests**: WDIO met Tauri-driver (Linux) en Appium Mac2 (macOS).
    -   **Rust-tests**: `cargo test` (core + Tauri-shell), met gedeelde mock-backend.
    -   Debug-wrappers in `scripts/debug/` voor compacte logs en persistente logbestanden.
    -   Coverage-merge gate: ≥ 80% coverage op gewijzigde regels (Vitest + `cargo-llvm-cov`).
-   **Workflow & ontwikkelrichtlijnen**
    
    -   Feature-ontwerp: eerst specificatie, dan Rust-implementatie, daarna JSON-RPC E2E, vervolgens UI en E2E in de desktopapp.
    -   Nieuwe functionaliteit in `src/openhuman/` altijd in een **eigen domeinmap** (geen losse `.rs`\-bestanden op rootniveau).
    -   Controller-schema’s en registries voor consistente JSON-RPC/CLI-exposure.
    -   Event bus (`src/core/event_bus/`) voor typed pub/sub en native request/response.
-   **Debugging & logging**
    
    -   Sterke nadruk op **uitgebreide debug- en trace-logging** in zowel Rust als TypeScript.
    -   Gebruik van gestructureerde, grep-vriendelijke logprefixen (bijv. `[domain]`, `[rpc]`, `[ui-flow]`).
    -   Geen logging van secrets, tokens of gevoelige PII.
-   **Tauri-shell & Rust-core specifieke instructies**
    
    -   Tauri-shell (`app/src-tauri/`) is een dunne host voor windowing, deep links, core-proceslifecycle en JSON-RPC-relay.
    -   Geregistreerde IPC-commando’s (zoals `core_rpc_relay`, `ai_get_config`, `ai_refresh_config`, `openhuman_*`) zijn gedocumenteerd in de Tauri-architectuurgids.
    -   Rust-core is opgesplitst in domeinen (`openhuman/<domain>/`) met duidelijke lagen: implementatie → controllers (`rpc.rs`) → transport (`core_server/`).

### Samenvattende tabel

| Aspect | Beschrijving |
| --- | --- |
| **Type applicatie** | Desktopapp (Windows, macOS, Linux) |
| **Frontend** | React + Vite in `app/`, Tailwind design system |
| **Desktop host** | Tauri v2 (`app/src-tauri/`) met CEF-ondersteuning |
| **Core** | Rust-library + CLI/sidecar (`openhuman_core` in `src/`) |
| **Skills-runtime** | QuickJS (`rquickjs`) in `src/openhuman/skills/` |
| **Communicatie** | JSON-RPC over HTTP tussen UI en core (`core_rpc_relay` / `core_rpc`) |
| **Tests** | Vitest (unit), WDIO (E2E), `cargo test` (Rust) |
| **Configuratie** | `.env.example` (root) + `app/.env.example` (frontend), TOML-config in Rust |
| **Belangrijke richtlijnen** | Sterke scheiding core/shell, hoge testcoverage, uitgebreide debug-logging, modulaire domeinstructuur |
| **Documentatiebronnen** | `gitbooks/developing/architecture*.md`, `AGENTS.md`, `CLAUDE.md`, `docs/` |